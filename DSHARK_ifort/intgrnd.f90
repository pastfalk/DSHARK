...
function intgrnd(z,n,h1,h2,h3,case)
  ...
  if ( case.eq. 1) then
     ...
     intgrnd = Bessel_Jn(n,h2*sqrt(z-1.0))*Bessel_Jn(n,h2*sqrt(z-1.0))/(z**h3) *Zk_func(h1/sqrt(z),h3-1)

  else if (case.eq.2) then
     !integrand appearing in epsilon_yy
     intgrnd = (z-1.0)*dBessel_Jn(n,h2*sqrt(z-1.0))*dBessel_Jn(n,h2*sqrt(z-1.0))/(z**h3) *Zk_func(h1/sqrt(z),h3-1)

  else if (case.eq.3) then
     !integrand appearing in epsilon_xy and epsilon_yz
     intgrnd = sqrt(z-1.0)*Bessel_Jn(n, h2*sqrt(z-1.0))*dBessel_Jn(n, h2*sqrt(z-1.0))/(z**h3) *Zk_func(h1/sqrt(z),h3-1)
