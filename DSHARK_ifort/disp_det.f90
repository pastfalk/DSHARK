     ...
     if (kappa(m).LT.50) then
        ...
        do n=-NBessel,NBessel
           ...
           epsilon_yy=epsilon_yy+ 2.0*sqrt(2.0) *sqrt(mu(m))*dens(m)**(3.0/2.0) *q(m)**2  *(kappa(m)-1.0/2.0)/&
                & sqrt(2.0*kappa(m)-3.0) *(kappa(m)+1)**(3.0/2.0) / (sqrt(beta_para(m)) *k*cos(theta)) * (beta_ratio(m) *&
           ...
           epsilon_xz=epsilon_xz+4.0*sqrt(2.0) * sqrt(mu(m)) *dens(m)**(5.0/2.0) * q(m)**3 * &
        ...
        do n=-NBessel,NBessel
           ...
           epsilon_xx=epsilon_xx + sqrt(mu(m)) *dens(m)**(3.0/2.0) * q(m)**2 / sqrt(beta_para(m)) / (k*cos(theta)) *&
                & exp(-lambda) * n**2 *Bessel_In(n,lambda) / lambda *&
