  ...
  do m=1,Nspecies
     ...
     if (kappa(m).LT.50) then
        ...
        do n=-NBessel,NBessel
           ...
           h1=sqrt((2.0*kappa(m)+2.0)/(2.0*kappa(m)-3.0)) * (omega-n*q(m)*mu(m))/(sqrt(beta_para(m))*k*cos(theta)*sqrt(mu(m)))
           h2=k*sin(theta)/sqrt(mu(m))/q(m) *sqrt((2.0*kappa(m)-3.0)*beta_perp(m)/2.0)
           h3=kappa(m)+2
...
!           write(*,*) m, n, lambda, Bessel_In(n,lambda)
           ...
           epsilon_xy = epsilon_xy + i*sqrt(mu(m))*q(m)**2 /(k*cos(theta)) / sqrt(beta_para(m)) *n*&
           ...
           epsilon_zz = epsilon_zz -  q(m)**2 *(omega-n*mu(m)*q(m)) / beta_perp(m) /(k*cos(theta))**2 *(omega*beta_ratio(m)-&
